...
export default function CRMIndex() {
    ...
    const stages = [
        { id: 'lead', title: 'Novos Leads', amount: 'R$ 150.000', count: 3, color: 'border-blue-500' },
        { id: 'contacted', title: 'Contatados', amount: 'R$ 290.000', count: 5, color: 'border-yellow-500' },
        { id: 'proposal', title: 'Proposta Enviada', amount: 'R$ 450.000', count: 4, color: 'border-orange-500' },
        { id: 'negotiation', title: 'Em Negociação', amount: 'R$ 800.000', count: 6, color: 'border-purple-500' },
        { id: 'won', title: 'Fechados/Ganhos', amount: 'R$ 1.200.000', count: 12, color: 'border-green-500' },
    ];
                ...
                <div className="flex space-x-6 h-full items-start">
                    {stages.map((stage) => (
                        <div key={stage.id} className="flex-shrink-0 w-80 flex flex-col max-h-full">
                            <div className={`bg-white dark:bg-gray-800 p-4 rounded-t-xl border-t-4 ${stage.color} shadow-sm mb-3 relative`}>
                                <div className="flex justify-between items-center mb-1">
                                    <h3 className="font-bold text-gray-800 dark:text-white">{stage.title}</h3>
                                ...
                                <div className="flex justify-between items-center text-sm text-gray-500 dark:text-gray-400">
                                    ...
                                    <span className="bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full text-xs font-medium">{stage.count} Negócios</span>
                                </div>
                            </div>
                            <div className="flex-1 overflow-y-auto space-y-3 pb-4 pr-1 scrollbar-hide">
                                {[1, 2, 3].map((card) => (
                                    <div key={card} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 cursor-pointer hover:shadow-md hover:border-indigo-300 transition-all group">
                                        <div className="flex justify-between items-start mb-2">
                                            <span className="px-2 py-1 bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400 text-[10px] font-bold uppercase rounded tracking-wider">Quente</span>
                                        ...
                                        <h4 className="font-bold text-gray-900 dark:text-white text-sm mb-1">Jeep Compass 2023 - João Silva</h4>
                                        <p className="text-lg font-bold text-gray-800 dark:text-gray-200 mb-3">R$ 145.000</p>
                                        <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                                            <div className="flex items-center text-xs text-gray-500 dark:text-gray-400"><Calendar className="w-3 h-3 mr-1" /> Há 2 dias</div>
                                            <div className="w-6 h-6 rounded-full bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center text-[10px] font-bold text-indigo-700 dark:text-indigo-300 ring-2 ring-white">JS</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
